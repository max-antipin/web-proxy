/* /js/pl_huioint.js */
$(function(){
	var addr = $('.pl_huioint_address'), body = $('#body');
	$('.pl_huioint_select_city').change(function(){
		var sel = $(this), data = {};
		data[sel.attr('name')] = sel.val();
		$.post('/', data, function(r){
			addr.text(r.address);
			if(r.image_url) body.css('background-image', 'url(' + r.image_url + ')');
		});
	});
});
/* /js/msfieldset.js */
